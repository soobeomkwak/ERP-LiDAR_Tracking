...
class RawInspector(Node):
    def __init__(self):
        super().__init__('raw_inspector')
        self.sub = self.create_subscription(PointCloud2, '/pointcloud/clustered', self.cb, 10)
        self.get_logger().info('RawInspector started')
    ...
    def cb(self, msg: PointCloud2):
        ...
        for i in range(cnt):
            ...
            try:
                ...
                break
            self.get_logger().info(f'pt[{i}]: x={x:.6f} y={y:.6f} z={z:.6f} cluster_int={cluster_int} rgb_f={rgb_f:.6f}')
        rclpy.shutdown()
